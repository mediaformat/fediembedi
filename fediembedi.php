...
class FediConfig {
	...
	public function mastodon_shortcode( $atts ) {
		...
		if ( false === ( $status = get_transient( "mastodon_$shortcode_atts" ) ) ) {
			...
			$client = self::fedi_client( $fedi_instance, 'mastodon' );
	...
	public function pixelfed_shortcode( $atts ) {
		...
		if ( false === ( $status = get_transient( "pixelfed_$shortcode_atts" ) ) ) {
			...
			$client = self::fedi_client( $fedi_instance, 'pixelfed' );
			$verify = $client->verify_credentials( $access_token );
			if ( is_wp_error( $verify ) ) {
				return;
			}
	...
	public function peertube_shortcode( $atts ) {
		...
		if ( false === ( $status = get_transient( "peertube_$shortcode_atts" ) ) ) {
			...
			$client = self::fedi_client( $atts['instance'] );
					...
					if ( empty( $instance ) ) {
						...
						switch ( $instance_type ) {
							case 'mastodon':
								...
								//$account = $client->verify_credentials( $mastodon_token );
								break;
							case 'pixelfed':
								...
								//$account = $client->verify_credentials( $pixelfed_token );
								break;
						}
				...
				update_option(
					...
					serialize(
						array( // TODO fix error translation handling, and clean up strings
							...
							'<p>' . sprintf(
								wp_kses(
									__( "Please <a href='%s'>re-authorize</a> your " . ucfirst( $type ) . ' account.', 'fediembedi' ),
									array( 'a' => array( 'href' => array() ) )
								),
								admin_url( 'options-general.php?page=fediembedi' )
							) . '</p>',
							'class' => 'error',
						)
					)
	...
	public function admin_notices() {
		...
		if ( in_array( $pagenow, $admin_pages ) ) {
			...
			if ( is_array( $notice ) ) {
				printf(
					'<div class="notice notice-fediembedi is-dismissible notice-%1$s"><p>%2$s</p></div>',
