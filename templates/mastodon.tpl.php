<!-- mastodon -->
<div class="fediembedi fediembedi-mastodon scrollable" <?php if ( ! empty( $height ) ) :
  ...
  <div role="feed">
	...
	<div class="account-timeline__header">
	  <div class="account__header">
		<div class="account__header__image">
		  ...
		  <?php
			if ( $account->header ) :
				echo '<img src=' . $account->header . " loading='lazy'>";
endif;
			?>
		</div>
	...
	echo $statut->reblog->url;
endif;
?>
" class="status__relative-time" target="_blank" rel="noopener">
					<time datetime="<?php echo $statut->created_at; ?>">
											   <?php
												printf(
				  ...
				  <?php
					if ( is_null( $statut->reblog ) ) :
						echo $statut->account->url;
else :
	echo $statut->reblog->account->url;
endif;
?>
" class="status__display-name" rel="noopener noreferrer" target="_blank">
					<div class="status__avatar">
					  <div class="account__avatar" style="background-image: url(
					  <?php
						if ( is_null( $statut->reblog ) ) :
							echo $statut->account->avatar;
else :
	echo $statut->reblog->account->avatar;
endif;
?>
); background-size: 40px; width: 40px; height: 40px;"></div>
					</div>
				...
				if ( empty( $statut->spoiler_text ) ) :
					...
					if ( ! is_null( $statut->card ) ) :
					  ...
					  echo '<details><summary>' . apply_filters( 'fedi_emoji', $statut->spoiler_text, $statut->emojis ) . '</summary>' . apply_filters( 'fedi_emoji', $statut->content, $statut->emojis ) . '</details>';
				  endif;
